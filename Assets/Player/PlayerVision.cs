	...
	void DeactivateLights()
	{
		for (int yIndex = 0; yIndex < arraySize; yIndex++)
		{
			...
			for (int xIndex = 0; xIndex < arraySize; xIndex++)
			{
				...
				if (LevelManager.S.realData[xReal, yReal].light == LightAmount.fogOfWar)
				{
					///LevelManager.S.realData[xReal, yReal].UpdateLight(LightAmount.fogOfWar);
				}
		...
		for (int step = 1; step < radius; step++) // Run one time less than the radius (No need to spread 1 value lights)
		{
			for (int yIndex = 0; yIndex < arraySize; yIndex++)
			{
				...
				for (int xIndex = 0; xIndex < arraySize; xIndex++)
				{
					...
					///if (LevelManager.S.realData[xReal, yReal].GetVisionObstruction() == VisionObstruction.open)
					///	SpreadLightAroundPos(new IntVector2(xIndex, yIndex), 1);
					///else if (LevelManager.S.realData[xReal, yReal].GetVisionObstruction() == VisionObstruction.limited)
	...
	void LightLevel()
	{
		for (int yIndex = 0; yIndex < arraySize; yIndex++)
		{
			...
			for (int xIndex = 0; xIndex < arraySize; xIndex++)
			{
				...
				if (xReal < 0 || xReal >= LevelManager.S.size.x)
					continue; // Out of bounds
				///if (lightValues[xIndex, yIndex] > 0)
				///	LevelManager.S.realData[xReal, yReal].UpdateLight(LightAmount.lit);
			}
		}
	}
}
